...
interface JobData {
  ...
  jobNumber: string;
  customerName: string;
  jobType: 'repair' | 'installation' | 'inspection' | 'maintenance';
...
export function EditJob({ job, onBack }: EditJobProps) {
  ...
  // Roofing specification fields (initialized with demo data)
  const [roofingSpecs, setRoofingSpecs] = useState({
    // Gutters
    gutterTotalFootage: '400\'',
    gutterType: '6"',
    gutterColor: 'Buckskin → Current By H/O',
    gutterSpecialtyItems: 'T-STRAPS',
    ...
    // Leaf Guard
    leafGuardSize: '6"',
    leafGuardTotalFootage: '400\'',
    leafGuardType: 'Roll',
    
    // Details
    storyOptions: ['1 Story'] as string[],
    crewCount: '',
    timeFrame: ''
  });

  const [scheduledDate, setScheduledDate] = useState<Date | undefined>(
    job.scheduledDate ? new Date(job.scheduledDate) : undefined
  );
  const [isCalendarOpen, setIsCalendarOpen] = useState(false);
  const [jobDrawing, setJobDrawing] = useState<string>('');

  // Demo existing photos
  const [jobPhotos, setJobPhotos] = useState<PhotoData[]>([
    {
      id: '1',
      file: new File([''], 'before-roof-damage.jpg', { type: 'image/jpeg' }),
      url: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&auto=format&fit=crop&q=60',
      category: 'before',
      filename: 'before-roof-damage.jpg',
      size: 245760,
      uploadDate: '2024-01-15T10:30:00Z',
      description: 'Damaged shingles on south side'
    },
    {
      id: '2',
      file: new File([''], 'before-gutter-issue.jpg', { type: 'image/jpeg' }),
      url: 'https://images.unsplash.com/photo-1628624747186-a941c476b7ef?w=800&auto=format&fit=crop&q=60',
      category: 'before',
      filename: 'before-gutter-issue.jpg',
      size: 189440,
      uploadDate: '2024-01-15T10:32:00Z',
      description: 'Clogged gutters before cleaning'
    },
    {
      id: '3',
      file: new File([''], 'after-roof-repair.jpg', { type: 'image/jpeg' }),
      url: 'https://images.unsplash.com/photo-1486304873000-235643847519?w=800&auto=format&fit=crop&q=60',
      category: 'after',
      filename: 'after-roof-repair.jpg',
      size: 312320,
      uploadDate: '2024-01-20T16:45:00Z',
      description: 'Completed roof repair work'
    }
  ]);
        ...
        <div className="max-w-full mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-600 mt-1">Update job information and progress</p>
            </div>
              ...
              <div>
                ...
                >
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Label htmlFor="installation" className="text-gray-700 cursor-pointer">Installation</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="inspection" id="inspection" />
                    <Label htmlFor="inspection" className="text-gray-700 cursor-pointer">Inspection</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="maintenance" id="maintenance" />
                    <Label htmlFor="maintenance" className="text-gray-700 cursor-pointer">Maintenance</Label>
                  </div>
                </RadioGroup>
              </div>
          ...
          <div className="mb-6">
            <PhotoUpload initialPhotos={jobPhotos} onPhotosChange={handlePhotosChange} />
          </div>

          {/* Drawing & Notes Section */}
          <div className="mb-6">
            <DrawingCanvas initialDrawing={jobDrawing} onSave={handleDrawingSave} />
          </div>
        </div>
      </main>
